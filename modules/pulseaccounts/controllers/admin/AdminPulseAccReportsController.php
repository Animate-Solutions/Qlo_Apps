...
class AdminPulseAccReportsController extends ModuleAdminController
{
    public function __construct()
    {
        $this->bootstrap = true;
        parent::__construct();
        $this->meta_title = $this->l('Accounting Reports');
    }
    ...
    public function initContent()
    {
        ...
        $data = $this->runReport($r, $from, $to);
        if (Tools::getValue('export')) {
            $rows = $this->flatten($r, $data);
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="' . $r . '-' . $from . '-' . $to . '.csv"');
        ...
        $this->context->smarty->assign(array(
            ...
            'accounts' => PulseAccService::accounts(null, false, true),
            'account_code' => Tools::getValue('account_code', '1210'),
            'year' => (int) Tools::getValue('year', date('Y')),
            'month' => Tools::getValue('month') !== false ? (int) Tools::getValue('month') : null,
            'date' => Tools::getValue('date', PulseAccService::bd()),
            'periods' => PulseAccService::periods(24),
    ...
    protected function runReport($r, $from, $to)
    {
        switch ($r) {
            case 'gl':
                return PulseAccReport::generalLedger(Tools::getValue('account_code', '1210'), $from, $to);
            case 'pl':
                return PulseAccReport::usaliPl($from, $to, (bool) Tools::getValue('compare'));
            case 'bs':
                return PulseAccReport::balanceSheet($to);
            case 'cf':
                return PulseAccReport::cashFlow($from, $to);
            case 'budget':
                return PulseAccReport::budgetVsActual((int) Tools::getValue('year', date('Y')), Tools::getValue('month') ? (int) Tools::getValue('month') : null);
            case 'rev_dept':
                return PulseAccReport::revenueByDepartment($from, $to);
            case 'rev_code':
                return PulseAccReport::revenueByChargeCode($from, $to);
            case 'drj':
                return PulseAccReport::dailyRevenueJournal(Tools::getValue('date', PulseAccService::bd()));
            default:
        ...
        switch ($r) {
            ...
            case 'pl':
                ...
                foreach ($data['departments'] as $k => $d) {
                    $rows[] = array('section' => $d['label'], 'line' => 'Revenue', 'amount' => $d['revenue']);
                    $rows[] = array('section' => $d['label'], 'line' => 'Cost of sales', 'amount' => $d['cost_of_sales']);
                    $rows[] = array('section' => $d['label'], 'line' => 'Payroll', 'amount' => $d['payroll']);
                    $rows[] = array('section' => $d['label'], 'line' => 'Other expense', 'amount' => $d['other']);
                    $rows[] = array('section' => $d['label'], 'line' => 'Departmental profit', 'amount' => $d['profit']);
                }
                foreach ($data['undistributed']['groups'] as $g) {
                    $rows[] = array('section' => 'Undistributed', 'line' => $g['label'], 'amount' => round($g['total'], 2));
                }
            ...
            case 'bs':
                ...
                foreach ($data['groups'] as $g) {
                    foreach ($g['rows'] as $x) {
                        $rows[] = array('section' => $g['label'], 'code' => $x['code'], 'name' => $x['name'], 'amount' => $x['amount']);
                    }
                    $rows[] = array('section' => $g['label'], 'code' => '', 'name' => 'Total ' . $g['label'], 'amount' => $g['total']);
                }
            ...
            case 'cf':
                ...
                foreach ($data['working_capital'] as $k => $v) {
                    $rows[] = array('line' => 'Movement in ' . $k, 'amount' => $v);
                }
            ...
            case 'drj':
                $rows = array();
                foreach ($data['gl'] as $g) {
                    $rows[] = array('section' => 'GL revenue', 'code' => $g['code'], 'name' => $g['name'], 'amount' => $g['amount']);
                }
                foreach ($data['settlements'] as $s) {
                    $rows[] = array('section' => 'Settlement', 'code' => $s['account_code'], 'name' => $s['account_name'], 'amount' => $s['amount']);
                }
