...
{
    const VERSION = '1.0.0';
    protected $tabs = array('AdminPulseCrm' => 'CRM', 'AdminPulseCrmGuests' => 'Guests', 'AdminPulseCrmSegments' => 'Segments', 'AdminPulseCrmCampaigns' => 'Campaigns',
        'AdminPulseCrmJourneys' => 'Journeys', 'AdminPulseCrmLoyalty' => 'Loyalty', 'AdminPulseCrmSurveys' => 'Surveys', 'AdminPulseCrmCases' => 'Service Recovery',
        'AdminPulseCrmReviews' => 'Reviews', 'AdminPulseCrmCorporate' => 'Corporate', 'AdminPulseCrmSettings' => 'CRM Settings');
    protected $hooks = array('displayBackOfficeHeader', 'moduleRoutes', 'actionPulseCheckIn', 'actionPulseCheckOut', 'actionPulseNoShow', 'actionPulseFolioPost',
        'actionPulseNightAuditClosed', 'actionPulseTicketCreated', 'actionValidateOrder', 'actionPulseCrmMemberEnrolled', 'actionPulseCrmPointsRedeemed',
        'actionPulseCrmSurveyCompleted', 'actionPulseCrmCaseOpened', 'actionPulseCrmReviewAdded');

    public function __construct()
    {
        $this->name = 'pulsecrm'; $this->tab = 'administration'; $this->version = self::VERSION; $this->author = 'Animate Solutions Limited';
        ...
        parent::__construct();
        $this->displayName = $this->l('Pulse CRM'); $this->description = $this->l('Guest 360 enrichment, segmentation, email/SMS/WhatsApp campaigns, event-triggered journeys, a tiered loyalty programme posting to the folio, NPS surveys, service-recovery cases with cost tracking, review management and corporate sales.');
    ...
    public function install()
    {
        ...
        $parent = (int) Tab::getIdFromClassName('AdminPulseCore'); $i = 120;
        ...
        foreach (array('PULSE_CRM_QUIET_FROM' => '21:00', 'PULSE_CRM_QUIET_TO' => '08:00', 'PULSE_CRM_SUPPRESS_DAYS' => 1, 'PULSE_CRM_IMPLIED_CONSENT' => 0,
            'PULSE_CRM_THROTTLE' => 100, 'PULSE_CRM_SEGMENT_CHUNK' => 20, 'PULSE_CRM_JOURNEY_CHUNK' => 100, 'PULSE_CRM_NPS_THRESHOLD' => 6,
            'PULSE_CRM_SURVEY_EXPIRY_DAYS' => 30, 'PULSE_CRM_REVIEW_URL' => '', 'PULSE_CRM_PORTAL_URL' => '', 'PULSE_CRM_BASE_URL' => '',
            'PULSE_CRM_MANAGER_EMAIL' => Configuration::get('PS_SHOP_EMAIL'), 'PULSE_CRM_SECRET' => Tools::passwdGen(48), 'PULSE_CRM_CRON_TOKEN' => Tools::passwdGen(32)) as $k => $v) { Configuration::updateValue($k, $v); }
        $this->seedProgram();
        $this->seedJourneys();
        return true;
    }
    ...
    public function uninstall()
    {
        ...
        foreach (array('QUIET_FROM', 'QUIET_TO', 'SUPPRESS_DAYS', 'IMPLIED_CONSENT', 'THROTTLE', 'SEGMENT_CHUNK', 'JOURNEY_CHUNK', 'NPS_THRESHOLD', 'SURVEY_EXPIRY_DAYS',
            'REVIEW_URL', 'PORTAL_URL', 'BASE_URL', 'MANAGER_EMAIL', 'SECRET', 'CRON_TOKEN') as $k) { Configuration::deleteByName('PULSE_CRM_'.$k); }
    ...
    {
        if (Db::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.'pulse_crm_loyalty_program`')) { return; }
        $id = PulseCrmLoyalty::saveProgram(array('code' => 'PULSE', 'name' => 'Pulse Rewards', 'point_value' => 1, 'min_redeem_points' => 2000, 'expiry_months' => 24,
            'qualify_window_months' => 12, 'enrol_bonus' => 500, 'terms' => 'Points are earned on net spend excluding VAT and consumption tax. Points expire 24 months after they are earned. 1 point = 1 naira on redemption.',
        ...
        ) as $t) { $t['id_pulse_crm_loyalty_program'] = $id; PulseCrmLoyalty::saveTier($t); }
    }
    ...
    {
        if (Db::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.'pulse_crm_journey`')) { return; }
        $j = array(
            array('code' => 'prearrival', 'name' => 'Pre-arrival upsell', 'trigger_event' => 'booking_confirmed', 'description' => 'Three days before arrival, offer the upgrade while there is still a room to give.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'T-3 pre-arrival note', 'delay_minutes' => 0, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_prearrival', 'subject' => 'Getting ready for you — {hotel}',
            ...
            array('code' => 'welcome', 'name' => 'Check-in welcome', 'trigger_event' => 'check_in', 'description' => 'Welcome note with the in-room portal link, fifteen minutes after the key is cut.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'Welcome', 'delay_minutes' => 15, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_welcome', 'subject' => 'Welcome to {hotel}, {first_name}',
            ...
            array('code' => 'midstay', 'name' => 'Mid-stay satisfaction ping', 'trigger_event' => 'mid_stay', 'description' => 'Night two: ask how it is going while it can still be fixed. A low score opens a ticket.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'How is everything?', 'delay_minutes' => 0, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_midstay', 'subject' => 'How is everything, {first_name}?',
                    'action_json' => array('survey' => 'in_stay'), 'body' => "Dear {first_name},\n\nYou are on night two with us. Is everything as it should be?\n\nThirty seconds here and the duty manager sees it tonight: {survey_url}"),
                array('sort' => 2, 'name' => 'Chase a detractor', 'delay_minutes' => 1440, 'action' => 'create_ticket', 'channel' => 'auto',
            ...
            array('code' => 'thankyou', 'name' => 'Check-out thank you and survey', 'trigger_event' => 'check_out', 'description' => 'Thank you two hours after departure, with the post-stay survey attached.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'Thank you + survey', 'delay_minutes' => 120, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_thankyou', 'subject' => 'Thank you for staying with us — {hotel}',
                    'action_json' => array('survey' => 'post_stay'), 'body' => "Dear {first_name},\n\nThank you for staying with us. It was good to have you.\n\nWould you tell us how we did? It takes under a minute and the general manager reads every one: {survey_url}"),
                array('sort' => 2, 'name' => 'Review request at T+7', 'delay_minutes' => 10080, 'action' => 'send_template', 'channel' => 'email', 'template_code' => 'crm_review_ask', 'subject' => 'Would you leave us a review?',
            ...
            array('code' => 'winback', 'name' => 'Win-back at 90 days', 'trigger_event' => 'lapsed', 'description' => 'Ninety days after the last stay, a members rate and a reason to come back.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'Win-back offer', 'delay_minutes' => 0, 'action' => 'send_template', 'channel' => 'email', 'template_code' => 'crm_winback', 'subject' => 'We have missed you at {hotel}',
            ...
            array('code' => 'birthday', 'name' => 'Birthday greeting', 'trigger_event' => 'birthday', 'description' => 'A greeting on the day, and 1,000 points for members.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'Greeting', 'delay_minutes' => 0, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_birthday', 'subject' => 'Happy birthday from all of us at {hotel}',
                ...
                array('sort' => 2, 'name' => 'Birthday points', 'delay_minutes' => 2, 'action' => 'add_points',
            ...
            array('code' => 'anniversary', 'name' => 'Anniversary greeting', 'trigger_event' => 'anniversary', 'description' => 'A note on the anniversary with dinner for two on the house.', 'active' => 1, 'steps' => array(
                array('sort' => 1, 'name' => 'Greeting', 'delay_minutes' => 0, 'action' => 'send_template', 'channel' => 'auto', 'template_code' => 'crm_anniversary', 'subject' => 'Happy anniversary — {hotel}',
                    'body' => "Dear {first_name},\n\nHappy anniversary from all of us.\n\nStay with us this month and dinner for two in the restaurant is on the house."),
            )),
        );
        foreach ($j as $spec) {
            $steps = $spec['steps']; unset($spec['steps']);
            $id = PulseCrmJourney::save($spec);
            foreach ($steps as $s) { $s['id_pulse_crm_journey'] = $id; PulseCrmJourney::saveStep($s); }
        }
    }

    public function getContent() { Tools::redirectAdmin($this->context->link->getAdminLink('AdminPulseCrm')); }
    ...
    {
        if (empty($p['customer']) || empty($p['order'])) { return; }
        try {
            ...
            if ($started && $delay > 0) { Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'pulse_crm_journey_run` SET next_run_at=DATE_ADD(NOW(), INTERVAL '.$delay.' MINUTE) WHERE id_customer='.$idc.' AND id_htl_booking='.(int) $b['id'].' AND status="active" AND step_index=0'); }
        } catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'order_hook_failed', array('error' => $e->getMessage())); }
    }

    public function hookActionPulseCheckIn($p)
    {
        if (empty($p['booking']['id_customer'])) { return; }
        try { PulseCrmJourney::trigger('check_in', array('id_customer' => (int) $p['booking']['id_customer'], 'id_htl_booking' => (int) $p['booking']['id'], 'id_room' => (int) $p['id_room'])); }
        catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'checkin_hook_failed', array('error' => $e->getMessage())); }
    }
    ...
    {
        if (empty($p['booking']['id_customer']) || !empty($p['room_move'])) { return; }
        try {
            $idc = (int) $p['booking']['id_customer'];
            if ($m = PulseCrmLoyalty::memberOf($idc)) { PulseCrmLoyalty::recalcTier((int) $m['id_pulse_crm_member']); }
            PulseCrmJourney::trigger('check_out', array('id_customer' => $idc, 'id_htl_booking' => (int) $p['booking']['id'], 'id_room' => (int) $p['id_room']));
        } catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'checkout_hook_failed', array('error' => $e->getMessage())); }
    }
    ...
    {
        if (empty($p['id_line']) || !empty($p['is_payment'])) { return; }
        try { PulseCrmLoyalty::earnFromFolioLine((int) $p['id_line']); }
        catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'earn_failed', array('line' => (int) $p['id_line'], 'error' => $e->getMessage())); }
    }
    ...
    {
        try {
            if (!PulseCrmService::tableExists('htl_booking_detail')) { return; }
        ...
        } catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'night_audit_hook_failed', array('error' => $e->getMessage())); }
    }
    ...
    {
        if (empty($p['id_ticket']) || empty($p['department']) || !PulseCrmService::tableExists('pulse_ticket')) { return; }
        try {
            $t = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'pulse_ticket` WHERE id_pulse_ticket='.(int) $p['id_ticket']);
            if (!$t || $t['category'] !== 'complaint' || $t['source'] === 'survey') { return; }
            if ((int) Db::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.'pulse_crm_case` WHERE id_pulse_ticket='.(int) $p['id_ticket'])) { return; }
            PulseCrmCase::open(array('source' => 'ticket', 'severity' => in_array($t['priority'], array('urgent', 'high')) ? 'high' : 'medium', 'department' => $t['department'],
                'id_customer' => $t['id_customer'], 'id_htl_booking' => $t['id_htl_booking'], 'id_room' => $t['id_room'], 'id_pulse_ticket' => (int) $p['id_ticket'],
                'title' => $t['title'], 'description' => $t['description']));
        } catch (Exception $e) { PulseCoreService::audit('pulsecrm', 'ticket_hook_failed', array('error' => $e->getMessage())); }
    }
