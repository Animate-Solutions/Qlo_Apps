...
class PulseCrmService
{
    ...
    public static function gp() { return self::tableExists('pulse_guest_profile'); }
    public static function co() { return self::tableExists('pulse_company'); }
    ...
    public static function cfg($k, $default = null) { $v = Configuration::get('PULSE_CRM_'.$k); return ($v === false || $v === '') ? $default : $v; }
    ...
    public static function token($bytes = 16) { return bin2hex(Tools::getBytes($bytes)); }

    /** HMAC over a payload with the module secret — used on click-redirects, where the URL travels in the query string. */
    public static function sign($payload) { return substr(hash_hmac('sha256', (string) $payload, self::cfg('SECRET', _COOKIE_KEY_)), 0, 32); }
    public static function verify($payload, $sig) { $good = self::sign($payload); return strlen($sig) === strlen($good) && hash_equals($good, (string) $sig); }
    ...
    {
        $from = $from ? $from : self::cfg('QUIET_FROM', '21:00'); $to = $to ? $to : self::cfg('QUIET_TO', '08:00');
        if (!$from || !$to || $from === $to) { return false; }
        try { $d = new DateTime($when ? $when : 'now', new DateTimeZone(self::TZ)); } catch (Exception $e) { return false; }
        $mins = (int) $d->format('G') * 60 + (int) $d->format('i');
        $f = self::minutes($from); $t = self::minutes($to);
        return $f <= $t ? ($mins >= $f && $mins < $t) : ($mins >= $f || $mins < $t);
    }
    protected static function minutes($hhmm) { $p = explode(':', $hhmm); return (int) $p[0] * 60 + (isset($p[1]) ? (int) $p[1] : 0); }
    ...
    public static function afterQuiet($from = null, $to = null)
    {
        ...
        if (!self::inQuietHours($from, $to)) { return date('Y-m-d H:i:s'); }
        try {
            $d = new DateTime('now', new DateTimeZone(self::TZ)); $p = explode(':', $to);
            $d->setTime((int) $p[0], isset($p[1]) ? (int) $p[1] : 0, 0);
            if ($d->getTimestamp() <= time()) { $d->modify('+1 day'); }
            $d->setTimezone(new DateTimeZone(date_default_timezone_get()));
            return $d->format('Y-m-d H:i:s');
        } catch (Exception $e) { return date('Y-m-d H:i:s', time() + 3600); }
    }
    ...
    public static function mergeVars($idCustomer, array $extra = array())
    {
        ...
        $v = array('hotel' => Configuration::get('PS_SHOP_NAME'), 'name' => $c->firstname, 'first_name' => $c->firstname, 'last_name' => $c->lastname,
            'full_name' => trim($c->firstname.' '.$c->lastname), 'email' => $c->email, 'city' => 'Port Harcourt', 'today' => date('d/m/Y'),
            'stays' => 0, 'nights' => 0, 'last_stay' => '', 'tier' => '', 'points' => 0, 'member_no' => '', 'room' => '', 'from' => '', 'to' => '');
        ...
        if ($gp) { $v['stays'] = (int) $gp['stays']; $v['nights'] = (int) $gp['nights']; $v['last_stay'] = $gp['last_stay']; $v['phone'] = $gp['phone']; }
        $m = PulseCrmLoyalty::memberOf($idCustomer);
        if ($m) { $v['tier'] = $m['tier_name']; $v['points'] = (int) $m['points_balance']; $v['member_no'] = $m['member_no']; }
        if (!empty($extra['id_htl_booking']) && self::fd() && class_exists('PulseFdService') && ($b = PulseFdService::booking((int) $extra['id_htl_booking']))) {
            $v['room'] = $b['room_num']; $v['from'] = $b['date_from']; $v['to'] = $b['date_to']; $v['ref'] = $b['order_ref']; $v['nights_stay'] = $b['nights'];
        }
    ...
    {
        foreach ($vars as $k => $val) { if (is_scalar($val)) { $text = str_replace('{'.$k.'}', $val, $text); } }
    ...
    public static function link($controller, array $params = array()) { return Context::getContext()->link->getModuleLink('pulsecrm', $controller, $params); }
    ...
    {
        $d = $date ? $date : self::bd();
        if (!self::fd() || !class_exists('PulseFdService') || !class_exists('HotelBookingDetail')) { return array(); }
        ...
        foreach ($rows as &$r) {
            ...
            $r['tier'] = $m ? $m['tier_name'] : ''; $r['points'] = $m ? (int) $m['points_balance'] : 0; $r['member_no'] = $m ? $m['member_no'] : '';
    ...
    {
        $db = Db::getInstance(); $from = date('Y-m-d', strtotime('-'.(int) $days.' day'));
        $nps = self::npsFor($from, date('Y-m-d'));
        return array(
            'nps' => $nps['nps'], 'nps_responses' => $nps['responses'], 'promoters' => $nps['promoters'], 'detractors' => $nps['detractors'],
    ...
    public static function npsFor($from, $to)
    {
        ...
        $n = (int) $r['responses'];
        return array('responses' => $n, 'promoters' => (int) $r['promoters'], 'detractors' => (int) $r['detractors'], 'gss' => (float) $r['gss'],
