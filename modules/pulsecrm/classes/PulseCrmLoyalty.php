...
class PulseCrmLoyalty
{
    ...
    public static function program($id = 0)
    {
        return $id
            ...
            : Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'pulse_crm_loyalty_program` WHERE active=1 ORDER BY id_pulse_crm_loyalty_program LIMIT 1');
    }
    ...
    public static function saveProgram(array $d, $id = 0)
    {
        ...
        $row = array('name' => pSQL($d['name']), 'earn_rate_json' => pSQL($rates, true), 'point_value' => (float) $d['point_value'],
            'min_redeem_points' => (int) $d['min_redeem_points'], 'expiry_months' => (int) $d['expiry_months'],
            'qualify_window_months' => (int) (isset($d['qualify_window_months']) ? $d['qualify_window_months'] : 12), 'enrol_bonus' => (int) (isset($d['enrol_bonus']) ? $d['enrol_bonus'] : 0),
            'terms' => pSQL(isset($d['terms']) ? $d['terms'] : '', true), 'active' => isset($d['active']) ? (int) $d['active'] : 1, 'date_upd' => date('Y-m-d H:i:s'));
        if ($id) { Db::getInstance()->update('pulse_crm_loyalty_program', $row, 'id_pulse_crm_loyalty_program='.(int) $id); return (int) $id; }
        $row['code'] = pSQL(isset($d['code']) ? Tools::strtoupper(Tools::substr($d['code'], 0, 16)) : 'PULSE'); $row['date_add'] = date('Y-m-d H:i:s');
    ...
    public static function programId() { $p = self::program(); return $p ? (int) $p['id_pulse_crm_loyalty_program'] : 0; }

    public static function saveTier(array $d, $id = 0)
    {
        $row = array('id_pulse_crm_loyalty_program' => (int) $d['id_pulse_crm_loyalty_program'], 'name' => pSQL($d['name']), 'sort' => (int) $d['sort'],
            'min_nights' => (int) $d['min_nights'], 'min_stays' => (int) $d['min_stays'], 'min_spend' => (float) $d['min_spend'],
            'earn_multiplier' => (float) $d['earn_multiplier'], 'benefits' => pSQL(isset($d['benefits']) ? $d['benefits'] : '', true), 'colour' => pSQL(isset($d['colour']) ? $d['colour'] : 'default'));
        if ($id) { Db::getInstance()->update('pulse_crm_tier', $row, 'id_pulse_crm_tier='.(int) $id); return (int) $id; }
    ...
    public static function byNumber($no) { return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'pulse_crm_member` WHERE member_no="'.pSQL($no).'"'); }
    ...
    public static function enrol($idCustomer, $source = 'desk', $idProgram = 0)
    {
        ...
        if (!$p) { throw new PrestaShopException('No active loyalty programme'); }
        if ($m = self::memberOf($idCustomer)) { return (int) $m['id_pulse_crm_member']; }
        $c = new Customer($idCustomer);
        if (!Validate::isLoadedObject($c)) { throw new PrestaShopException('No such guest'); }
        ...
        Db::getInstance()->insert('pulse_crm_member', array('id_customer' => $idCustomer, 'id_pulse_crm_loyalty_program' => (int) $p['id_pulse_crm_loyalty_program'],
            'member_no' => pSQL($no), 'card_no' => pSQL(self::cardNo($no)), 'id_pulse_crm_tier' => $base, 'tier_since' => date('Y-m-d'),
            'tier_review_date' => date('Y-m-d', strtotime('+'.(int) $p['qualify_window_months'].' month')), 'join_date' => date('Y-m-d'),
            'enrol_source' => pSQL($source), 'date_add' => date('Y-m-d H:i:s'), 'date_upd' => date('Y-m-d H:i:s')));
        $id = (int) Db::getInstance()->Insert_ID();
        if ((int) $p['enrol_bonus'] > 0) { self::award($id, 'bonus', (int) $p['enrol_bonus'], 'enrolment', 'Welcome bonus', array()); }
    ...
    {
        $digits = preg_replace('/\D/', '', $memberNo); $digits = str_pad(substr($digits, -11), 11, '0', STR_PAD_LEFT);
        $sum = 0; $alt = true;
        for ($i = strlen($digits) - 1; $i >= 0; $i--) { $d = (int) $digits[$i]; if ($alt) { $d *= 2; if ($d > 9) { $d -= 9; } } $sum += $d; $alt = !$alt; }
    ...
    public static function award($idMember, $type, $points, $source, $description, array $extra = array())
    {
        $points = (int) $points;
        if (!$points) { return 0; }
        $m = self::member($idMember); if (!$m) { throw new PrestaShopException('No such member'); }
        ...
        if ($balance < 0) { throw new PrestaShopException('That would take the balance below zero'); }
        ...
        Db::getInstance()->insert('pulse_crm_points_txn', array('id_pulse_crm_member' => (int) $idMember, 'type' => pSQL($type), 'points' => $signed,
            'points_remaining' => $isLot ? abs($signed) : 0, 'balance_after' => $balance, 'source' => pSQL($source), 'reference' => pSQL(isset($extra['reference']) ? $extra['reference'] : ''),
            'description' => pSQL($description), 'id_htl_booking' => !empty($extra['id_htl_booking']) ? (int) $extra['id_htl_booking'] : null,
            'id_pulse_folio_line' => !empty($extra['id_pulse_folio_line']) ? (int) $extra['id_pulse_folio_line'] : null, 'department' => pSQL(isset($extra['department']) ? $extra['department'] : ''),
            'amount_basis' => (float) (isset($extra['amount_basis']) ? $extra['amount_basis'] : 0), 'expires_on' => $isLot ? pSQL($expiry) : null,
            'id_employee' => PulseCrmService::emp(), 'business_date' => PulseCrmService::bd(), 'date_add' => date('Y-m-d H:i:s')));
        $id = (int) Db::getInstance()->Insert_ID();
        ...
        if ($signed < 0 && $type !== 'expire') { self::consumeLots($idMember, abs($signed)); }
        return $id;
    }
    ...
    public static function rateFor($program, $department)
    {
        ...
        if (isset($rates[$department])) { return (float) $rates[$department]; }
    ...
    {
        if (!PulseCrmService::fd() || !PulseCrmService::tableExists('pulse_folio_line')) { return 0; }
        ...
        if (!$l || (int) $l['is_payment'] || (int) $l['voided'] || !$l['id_customer']) { return 0; }
        if (in_array($l['code'], array('LOYR', 'ADJ')) || in_array($l['department'], array('payment', 'tax', 'adjustment'))) { return 0; }
        if ((float) $l['amount_tax_incl'] <= 0) { return 0; }
        if ((int) Db::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.'pulse_crm_points_txn` WHERE id_pulse_folio_line='.(int) $idLine)) { return 0; }
        $m = self::memberOf((int) $l['id_customer']); if (!$m) { return 0; }
        ...
        self::award((int) $m['id_pulse_crm_member'], 'earn', $points, 'folio', 'Earned on '.$l['description'], array(
            'id_pulse_folio_line' => (int) $idLine, 'id_htl_booking' => (int) $l['id_htl_booking'], 'department' => $l['department'],
            'amount_basis' => $base, 'reference' => $l['code']));
        return $points;
    }
    ...
    public static function redeem($idMember, $points, $idBooking = null, $note = '')
    {
        $m = self::member($idMember); if (!$m) { throw new PrestaShopException('No such member'); }
        $points = (int) $points;
        if ($points < (int) $m['min_redeem_points']) { throw new PrestaShopException('Minimum redemption is '.(int) $m['min_redeem_points'].' points'); }
        if ($points > (int) $m['points_balance']) { throw new PrestaShopException('Balance is only '.(int) $m['points_balance'].' points'); }
        $value = round($points * (float) $m['point_value'], 2);
        $line = null;
        if ($idBooking && PulseCrmService::fd()) {
            $f = PulseFolio::openForBooking((int) $idBooking);
            if (!$f) { throw new PrestaShopException('That stay has no open folio to credit'); }
        ...
        $id = self::award($idMember, 'redeem', $points, 'redemption', 'Redeemed '.$points.' points'.($idBooking ? ' on stay '.(int) $idBooking : ''), array(
            'id_htl_booking' => $idBooking ? (int) $idBooking : null, 'id_pulse_folio_line' => $line, 'reference' => $note));
    ...
    {
        if (!trim($reason)) { throw new PrestaShopException('A manual adjustment needs a reason'); }
    ...
    {
        $m = self::member($idMember); if (!$m) { return false; }
        $p = self::program((int) $m['id_pulse_crm_loyalty_program']);
        ...
        if (PulseCrmService::tableExists('htl_booking_detail')) {
            ...
            if ($q) { $r = array('nights' => (int) $q['nights'], 'stays' => (int) $q['stays'], 'spend' => (float) $q['spend']); }
        }
    ...
    public static function recalcTier($idMember, $allowDowngrade = false)
    {
        ...
        $target = null;
        foreach ($tiers as $t) {
            if ((int) $m['qualifying_nights'] >= (int) $t['min_nights'] && (int) $m['qualifying_stays'] >= (int) $t['min_stays'] && (float) $m['qualifying_spend'] >= (float) $t['min_spend']) { $target = $t; }
        }
        if (!$target) { $target = $tiers ? $tiers[0] : null; }
        ...
        if ((int) $target['id_pulse_crm_tier'] === $cur) { return false; }
        if (!$up && !$allowDowngrade && $m['tier_review_date'] && $m['tier_review_date'] > date('Y-m-d')) { return false; }
        Db::getInstance()->update('pulse_crm_member', array('id_pulse_crm_tier' => (int) $target['id_pulse_crm_tier'], 'tier_since' => date('Y-m-d'),
            'tier_review_date' => date('Y-m-d', strtotime('+12 month')), 'date_upd' => date('Y-m-d H:i:s')), 'id_pulse_crm_member='.(int) $idMember);
        if ($up) {
    ...
    public static function recalcTiersDue($limit = 200)
    {
        ...
        foreach (Db::getInstance()->executeS('SELECT id_pulse_crm_member FROM `'._DB_PREFIX_.'pulse_crm_member` WHERE status="active" AND (tier_review_date IS NULL OR tier_review_date<=CURDATE() OR date_upd<DATE_SUB(NOW(), INTERVAL 7 DAY)) ORDER BY COALESCE(tier_review_date,"1970-01-01") LIMIT '.(int) $limit) as $m) {
            if (self::recalcTier((int) $m['id_pulse_crm_member'], true)) { $n++; }
        }
        return $n;
    }
    ...
    public static function expirePoints($limit = 500)
    {
        ...
        foreach (Db::getInstance()->executeS('SELECT id_pulse_crm_member, SUM(points_remaining) pts FROM `'._DB_PREFIX_.'pulse_crm_points_txn`
            ...
            $pts = (int) $r['pts']; if ($pts <= 0) { continue; }
            $m = self::member((int) $r['id_pulse_crm_member']); if (!$m) { continue; }
            $pts = min($pts, (int) $m['points_balance']);
            if ($pts > 0) { self::award((int) $r['id_pulse_crm_member'], 'expire', $pts, 'expiry', $pts.' points expired', array()); $expired += $pts; }
