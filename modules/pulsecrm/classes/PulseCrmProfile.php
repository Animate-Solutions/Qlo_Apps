...
class PulseCrmProfile
{
    ...
    {
        if (class_exists('PulseGuestProfile')) {
            PulseGuestProfile::touch((int) $idCustomer);
        }
    ...
    public static function saveExt($idCustomer, array $d)
    {
        ...
        foreach (array('source_of_business', 'market_segment', 'guest_type', 'preferred_language', 'preferred_channel') as $k) {
            if (isset($d[$k])) {
                $u[$k] = pSQL($d[$k]);
            }
        }
    ...
    public static function savePreference($idCustomer, $category, $value, $code = null, $source = 'desk')
    {
        ...
        if (!in_array($category, self::CATEGORIES)) {
            throw new PrestaShopException('Unknown preference category');
        }
        $id = (int) $idCustomer;
        $value = trim($value);
        if ($code && !$value) {
            $o = Db::getInstance()->getRow('SELECT label FROM `' . _DB_PREFIX_ . 'pulse_crm_preference_option` WHERE category="' . pSQL($category) . '" AND code="' . pSQL($code) . '"');
            $value = $o ? $o['label'] : $code;
        }
        if (!$value) {
            throw new PrestaShopException('A preference needs a value');
        }
        ...
        if ($exists) {
            Db::getInstance()->update('pulse_crm_preference', array('active' => 1, 'date_upd' => date('Y-m-d H:i:s')), 'id_pulse_crm_preference=' . $exists);
            return $exists;
        }
        self::touch($id);
        Db::getInstance()->insert('pulse_crm_preference', array(
            'id_customer' => $id,
            'category' => pSQL($category),
            'code' => pSQL($code),
            'value' => pSQL($value),
            'is_managed' => $code ? 1 : 0,
            'is_service_note' => $note,
            'source' => pSQL($source),
    ...
    public static function removePreference($idPreference)
    {
        ...
        if ($p) {
            self::mirrorToFd((int) $p['id_customer']);
        }
        return true;
    }
    ...
    protected static function mirrorToFd($idCustomer)
    {
        ...
        $out = array();
        foreach (Db::getInstance()->executeS('SELECT category, value FROM `' . _DB_PREFIX_ . 'pulse_crm_preference` WHERE id_customer=' . (int) $idCustomer . ' AND active=1') as $r) {
    ...
    public static function saveOccasion($idCustomer, $type, $date, $note = '', $remindDays = 7, $recurring = 1)
    {
        if (!Validate::isDate($date)) {
            throw new PrestaShopException('Occasion date is not a date');
        }
        self::touch($idCustomer);
        ...
        if ($exists) {
            Db::getInstance()->update('pulse_crm_occasion', $d, 'id_pulse_crm_occasion=' . $exists);
            return $exists;
        }
        $d['id_customer'] = (int) $idCustomer;
        $d['source'] = 'desk';
    ...
    public static function removeOccasion($id)
    {
        return Db::getInstance()->update('pulse_crm_occasion', array('active' => 0, 'date_upd' => date('Y-m-d H:i:s')), 'id_pulse_crm_occasion=' . (int) $id);
    }
    ...
    {
        if ((int) $idCustomer === (int) $idRelated) {
            throw new PrestaShopException('A guest cannot be related to themselves');
        }
        ...
        if ($reciprocal && isset($mirror[$type])) {
            Db::getInstance()->execute('INSERT IGNORE INTO `' . _DB_PREFIX_ . 'pulse_crm_relationship` (id_customer, id_related_customer, type, note, date_add) VALUES (' . (int) $idRelated . ',' . (int) $idCustomer . ',"' . pSQL($mirror[$type]) . '","' . pSQL($note) . '",NOW())');
        }
        return true;
    }
    public static function unrelate($id)
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'pulse_crm_relationship` WHERE id_pulse_crm_relationship=' . (int) $id);
    }
    ...
    public static function consent($idCustomer)
    {
        ...
        foreach (array('email', 'sms', 'whatsapp', 'phone', 'post', 'profiling') as $ch) {
            $out[$ch] = array('channel' => $ch, 'state' => 'unknown', 'source' => '', 'date_consent' => null, 'unsub_reason' => null);
        }
        foreach (Db::getInstance()->executeS('SELECT * FROM `' . _DB_PREFIX_ . 'pulse_crm_consent` WHERE id_customer=' . (int) $idCustomer) as $r) {
            $out[$r['channel']] = $r;
        }
        return $out;
    }
    ...
    {
        if (!in_array($channel, array('email', 'sms', 'whatsapp', 'phone', 'post', 'profiling'))) {
            throw new PrestaShopException('Unknown consent channel');
        }
        if (!in_array($state, array('opt_in', 'opt_out', 'unknown'))) {
            throw new PrestaShopException('Unknown consent state');
        }
        self::touch($idCustomer);
    ...
    {
        $id = (int) $idCustomer;
        if (PulseCrmService::tableExists('pulse_guest_profile') && (int) Db::getInstance()->getValue('SELECT blacklisted FROM `' . _DB_PREFIX_ . 'pulse_guest_profile` WHERE id_customer=' . $id)) {
            return 'blacklisted';
        }
        if ((int) Db::getInstance()->getValue('SELECT forgotten FROM `' . _DB_PREFIX_ . 'pulse_crm_profile_ext` WHERE id_customer=' . $id)) {
            return 'forgotten';
        }
        ...
        if ($state !== 'opt_in') {
            return PulseCrmService::cfg('IMPLIED_CONSENT', 0) ? true : 'no_consent';
        }
        return true;
    }
    ...
    public static function tag($idCustomer, $code, $source = 'journey')
    {
        ...
        if (!$id) {
            Db::getInstance()->insert('pulse_crm_tag', array('code' => pSQL($code), 'name' => pSQL(ucfirst(str_replace('_', ' ', $code))), 'colour' => 'default'));
    ...
    public static function forget($idCustomer, $reason = 'Guest exercised the right to erasure')
    {
        ...
        foreach (array('email', 'sms', 'whatsapp', 'phone', 'post', 'profiling') as $ch) {
            self::setConsent($id, $ch, 'opt_out', 'forget_request', 'right to erasure', $reason);
        }
