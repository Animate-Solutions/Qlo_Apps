...
class PulseKeycard extends Module
{
    ...
    {
        $this->name = 'pulsekeycard'; $this->tab = 'administration'; $this->version = self::VERSION; $this->author = 'Animate Solutions Limited';
        ...
        parent::__construct();
        $this->displayName = $this->l('Pulse Key Card'); $this->description = $this->l('Door key encoding for Onity, Salto, Hune and Dormakaba locks with a hardware-free simulator: guest and duplicate keys, mobile BLE/QR keys, staff access groups, lock audit trail and battery alerts.');
    ...
    public function install()
    {
        ...
        $parent = (int) Tab::getIdFromClassName('AdminPulseCore'); $i = 90;
        ...
        foreach (array('PULSE_KC_ADAPTER' => 'PulseKcAdapterSimulator', 'PULSE_KC_AUTO_ISSUE' => 1, 'PULSE_KC_EARLY_GRACE_HRS' => 2, 'PULSE_KC_LATE_GRACE_HRS' => 2,
            'PULSE_KC_MAX_DUPLICATES' => 4, 'PULSE_KC_TIMEOUT' => 8, 'PULSE_KC_MOBILE_ENABLED' => 1, 'PULSE_KC_MOBILE_TTL_MIN' => 240, 'PULSE_KC_MOBILE_REBIND' => 0,
            'PULSE_KC_MOBILE_MAX_REFRESH' => 500, 'PULSE_KC_AUDIT_RETENTION' => 180, 'PULSE_KC_ENCODER_STALE_HRS' => 6, 'PULSE_KC_BATTERY_PCT' => 20,
            'PULSE_KC_STAFF_CARD_DAYS' => 90, 'PULSE_KC_LOCAL_AGENT' => 1, 'PULSE_KC_AGENT_PORT' => 7070, 'PULSE_KC_SECRET' => Tools::passwdGen(48), 'PULSE_KC_CRON_TOKEN' => Tools::passwdGen(32)) as $k => $v) { Configuration::updateValue($k, $v); }
        if (class_exists('PulseKcService')) { PulseKcService::syncRoomDoors(); }
        return true;
    }
    ...
    public function uninstall()
    {
        ...
        foreach (array('ADAPTER', 'AUTO_ISSUE', 'EARLY_GRACE_HRS', 'LATE_GRACE_HRS', 'MAX_DUPLICATES', 'TIMEOUT', 'MOBILE_ENABLED', 'MOBILE_TTL_MIN', 'MOBILE_REBIND', 'MOBILE_MAX_REFRESH', 'AUDIT_RETENTION', 'ENCODER_STALE_HRS', 'BATTERY_PCT', 'STAFF_CARD_DAYS', 'LOCAL_AGENT', 'AGENT_PORT', 'SECRET', 'CRON_TOKEN') as $k) { Configuration::deleteByName('PULSE_KC_'.$k); }
    ...
    public function getContent() { Tools::redirectAdmin($this->context->link->getAdminLink('AdminPulseKeycard')); }
    ...
    public function hookModuleRoutes() { return array('pulsekeycard-api' => array('controller' => 'api', 'rule' => 'pulse/api/keycard{/:resource}{/:id}', 'keywords' => array('resource' => array('regexp' => '[a-z_]+', 'param' => 'resource'), 'id' => array('regexp' => '[0-9]+', 'param' => 'id')), 'params' => array('fc' => 'module', 'module' => $this->name))); }
    ...
    {
        if (empty($p['booking']['id']) || !Configuration::get('PULSE_KC_AUTO_ISSUE')) { return; }
        if (!empty($p['room_move'])) { return; } // the room-move hook already re-cut the key
        try { PulseKcKey::issueForBooking((int) $p['booking']['id']); }
        catch (PulseKcEncoderException $e) { PulseKcService::alert('Auto key for room '.$p['booking']['room_num'].' failed — '.$e->userMessage(), (int) $p['booking']['id'], (int) $p['id_room']); }
        catch (Exception $e) { PulseCoreService::audit('pulsekeycard', 'auto_issue_failed', array('booking' => (int) $p['booking']['id'], 'error' => $e->getMessage())); }
    }
    ...
    public function hookActionPulseRoomMove($p)
    {
        ...
        try {
            if (!empty($p['from_room'])) { PulseKcKey::cancelForRoom((int) $p['from_room'], 'Room move — guest left '.(int) $p['from_room']); }
            if (Configuration::get('PULSE_KC_AUTO_ISSUE')) { PulseKcKey::issueForBooking((int) $p['booking']['id'], array('rooms' => array((int) $p['to_room']))); }
        ...
        } catch (Exception $e) { PulseCoreService::audit('pulsekeycard', 'room_move_key_failed', array('booking' => (int) $p['booking']['id'], 'error' => $e->getMessage())); }
    }
    ...
    public function hookActionPulseStayChanged($p)
    {
        ...
        try {
            ...
            if (!empty($r['failed'])) { PulseKcService::alert($r['failed'].' card(s) for room '.$p['booking']['room_num'].' could not be re-encoded to '.$r['valid_to'].' — ask the guest to bring the card to the desk.', (int) $p['booking']['id'], (int) $p['booking']['id_room']); }
        } catch (Exception $e) { PulseCoreService::audit('pulsekeycard', 'stay_change_key_failed', array('booking' => (int) $p['booking']['id'], 'error' => $e->getMessage())); }
    }
    ...
    public function hookActionPulseCheckOut($p)
    {
        ...
        } catch (Exception $e) { PulseCoreService::audit('pulsekeycard', 'checkout_cancel_failed', array('booking' => (int) $p['booking']['id'], 'error' => $e->getMessage())); }
    }
