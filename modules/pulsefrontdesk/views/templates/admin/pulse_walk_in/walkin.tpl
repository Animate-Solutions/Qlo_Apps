...
$('#wi-quote').click(function(){ $.post(A,{ajax:1,action:'Quote',from:$('#wi-from').val(),to:$('#wi-to').val()},function(d){ types=d.types; var h='';
 ...
 $('#wi-types').html(h); var pf=
{/literal}{$prefill|json_encode}{literal}
; if(pf.id_product){ $('.wi-qty[data-type='+pf.id_product+']').val(1); if(pf.id_room) $('.wi-room[data-type='+pf.id_product+']').val(pf.id_room); } summary(); },'json'); });
